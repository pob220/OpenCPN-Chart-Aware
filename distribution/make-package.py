        ...
        if path.is_file() and not path.is_symlink():
            with path.open("rb") as stream:
                if stream.read(4) == b"\x7fELF":
                    elf.append(path)
    directories = sorted({str(p.parent) for p in elf})
    env = dict(os.environ, LD_LIBRARY_PATH=":".join(directories))
...
def main(work):
    ...
    version = os.environ.get("PREVIEW_VERSION", "5.14.0+chartaware.20260910.1")
    root = work / "deb-root"
    if root.exists():
        raise RuntimeError(f"Refusing to overwrite package staging: {root}")
    shutil.copytree(work / "stage", root, symlinks=True)
    shutil.copytree(work / "plugin-stage", root, dirs_exist_ok=True, symlinks=True)
    prefix = root / PREFIX
